...
stochastic.hill.climbing <- function(problem,
  ...
  name_method      <- "Stochastic Hill Climbing"
  ...
  while (count <= max_iterations) {
    ...
    sucesores_candidatos <- list()
    evals <- sapply(successor_nodes, function(x) x$evaluation)
    
    for (i in 1:length(evals)) {
      if (evals[i] < node_current$evaluation) {
        sucesores_candidatos[[length(sucesores_candidatos) + 1]] <- successor_nodes[[i]]
      }
    }
    
    if (length(sucesores_candidatos) == 0) {
      node_best_successor <- node_current
    } else {
      idx <- sample(1:length(sucesores_candidatos), 1)
      node_best_successor <- sucesores_candidatos[[idx]]
    }
